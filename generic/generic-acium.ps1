...
.NOTES
    ...
    version hardcodes the download URL directly in SECTION 1 below. Bumping
    to a new agent version means editing $DownloadUrl in this script and
    redeploying it — there is no external variable to update instead. This
    trades "edit a variable, not the script" for "works identically under
    any RMM platform (or run manually), since nothing depends on a
    platform-specific variable/parameter mechanism."
...
# failure — someone editing the script left $DownloadUrl empty.
if (-not $DownloadUrl) {

...
.NOTES
    ...
    Datto RMM Component Variables expected (set these in the Component UI
    ...
    Datto exposes Component Variables to the script as environment
    variables at runtime.

        AgentDownloadUrl    - REQUIRED. Direct URL to the pinned agent zip,
                              e.g. https://storage.googleapis.com/ebm-sensors-prod/win/acium-sensor-setup.zip

        AgentOrganization   - OPTIONAL. The organization/tenant ID these
                              endpoints report under, passed to the MSI as
                              the ORGANIZATION property. Set this per site
                              or per customer. Leave unset to install
                              without it.

        ExpectedPublisherCN - OPTIONAL. Expected Authenticode signing
                              certificate subject CN. When set, an MSI that
                              isn't validly signed by it is refused (exit
                              6). Leave unset to log the signature status
...
# =========================================================================
# SECTION 1: CONFIG
# Set up file paths and read in the settings Datto RMM passes to the
...
#
# Datto surfaces Component Variables to the script as environment
# variables. Configure these in the Component UI rather than editing this
# file, so a version bump or a new customer's org ID is a variable change,
# not a code change.
...
# to the MSI as the ORGANIZATION property. Set it per site/customer in the
# Component or Job. Leave the variable unset to install without it.
$Organization = $env:AgentOrganization
...
# whatever Datto RMM happened to capture from that one run.
#
...
# definitive way to confirm whether Datto RMM is truly executing as SYSTEM
...
if (-not $DownloadUrl) {
    Write-Log "ERROR: Missing required Component Variable (AgentDownloadUrl)." 'ERROR'
    exit 4
}
...
if ($Organization -match '"') {
    Write-Log "ERROR: the AgentOrganization Component Variable contains a double quote, which cannot be passed to msiexec safely." 'ERROR'
    exit 4
}
...
# shouldn't surface as a deployment failure in the Datto RMM dashboard.
...
try {
    ...
    if ($ExpectedPublisherCN) {
        ...
        Write-Log "MSI is not validly signed ($($signature.Status)). Continuing because the ExpectedPublisherCN Component Variable is not set - set it to enforce this." 'WARN'
    } else {
        Write-Log "MSI is validly signed. Set the ExpectedPublisherCN Component Variable to '$signerCN' to enforce this on every run." 'WARN'
    }
} catch {
    if ($ExpectedPublisherCN) {
